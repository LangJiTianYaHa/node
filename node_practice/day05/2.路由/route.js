...
app.get('/demo',function (request,response) {
  ...
  // response.end('我是一级demo路由')    //end 不会追加响应头
  // response.redirect('https://www.baidu.com')
  // response.send('我是一级demo路由')
  // response.sendFile(__dirname+'/public/vue.png')
  // response.set('demo','hello world')
  // response.send('我是一级demo路由')
...
app.get('/demo/test',function (request,response) {
  response.send('我是二级test路由')
})
app.get('/meishi/:id',function (request,response) {
  console.log(request.params)
  let {id} = request.params
  response.send(`我是变化的的${id}路由`)
})
app.post('/',function (request,response) {
  response.send('我是跟路由返回的数据 ---------- get')
})
