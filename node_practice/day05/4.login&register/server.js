  ...
  */
   ...
   * */
    ...
    if(!emailReg.test(email)){
      res.send('邮箱输入不合法，用户名必须为4-20，主机名必须为2-10位')
    }else if(!passwordReg.test(password)){
      res.send('密码格式不合法，必须为6-10位')
      ...
      usersModel.findOne({email},function (err,data) {
        if(data){
          //如果注册过
          console.log(`邮箱注册失败，因为${email}已经注册过`)
          res.send(`该邮箱已经被注册，请更换邮箱`)
        }else{
          //如果未注册过
          usersModel.create({email,nick_name,password},function (err) {
            if(!err){
              console.log(`邮箱为${email}的用户已经注册成功`)
              res.send('注册成功')
            }else{
              console.log(err)
              res.send('您当前网络不稳定，稍后重试')
            }
          })
        }
      })
