...
//连接数据库
mongoose.connect('mongodb://localhost:27017/demo6',{
...
mongoose.connection.on('open',function (err) {
  if(err) console.log('数据库连接失败',err)
    ...
    //2.制定进入你家的规则 --------  创建约束对象
    let schoolRule = new Schema({
      stu_id:{  //限制学号为：字符串  标识唯一
        required:true,
        type:String,
        unique: true
      },
    ...
    //3.告诉保安你的规则 ------- 创建模型对象
    let stuModel = mongoose.model('students',schoolRule)
    ...
    /*find方法：
       ...
       2.若没有结果，返回一个null
   */
    /*stuModel.find({name:'张三'},{_id:0},function (err,data) {
      if(!err) console.log(data)
      else console.log(err)
    })*/

    //更新
   /* stuModel.updateOne({name:'静静'},{age:29,info:'我爱写代码'},function (err,data) {
      if(!err) console.log(data)
      else console.log(err)
    })*/
